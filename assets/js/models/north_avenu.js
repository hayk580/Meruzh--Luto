class Game {
  ...
  constructor(canvasId) {
    ...
    this.bag = new Bag(this.ctx , 4500, this.canvas.height-350)
    this.shaurma = new Shaurma(this.ctx, this.bag.x+20, this.bag.y-30)
  ...
  start() {
    localStorage.setItem("level", "index15.html")
...
for (let i = 0; i < this.mario.bulletsleft.length; i++) {
  ...
  this.inteligent = KodrvacIntel
    ...
    if(this.alvards.length == 0) {
        this.alvards.push(
            new Alvard(this.ctx, this.canvas.width + 330, this.canvas.height - 120),
            new Alvard(this.ctx, this.canvas.width + 670, this.canvas.height - 120),
            new Alvard(this.ctx, this.canvas.width + 1090, this.canvas.height - 120),
            )
    }
    ...
    this.alvards.forEach(alvard => {
        ...
        if(alvard.x < 0) {
            this.alvards.shift()
            this.alvards.push(new Alvard(this.ctx, this.canvas.width + 250, this.canvas.height - 120))
        }
    })
    ...
    if(this.polices.length == 0) {
        this.polices.push(
            ...
            new Police(this.ctx, this.canvas.width + 1700, this.canvas.height - 120)
            )
    }
