class Game {
  ...
  constructor(canvasId) {
    ...
    this.background = new Background(this.ctx, this.levels.level13)

    this.bag = new Bag(this.ctx, 5000, this.canvas.height- 300)
  ...
  start() {
    localStorage.setItem("level", "index13.html")
...
for (let i = 0; i < this.mario.bulletsleft.length; i++) {
  ...
  this.inteligent = KodrvacIntel
    ...
    if(this.alvards.length == 0) {
        this.alvards.push(
            new Alvard(this.ctx, this.canvas.width + 170, this.canvas.height - 120),
            new Alvard(this.ctx, this.canvas.width + 360, this.canvas.height - 120),
            new Alvard(this.ctx, this.canvas.width + 770, this.canvas.height - 120),
            )
    }
    ...
    this.alvards.forEach(alvard => {
        ...
        if(alvard.x < 0) {
            this.alvards.shift()
            this.alvards.push(new Alvard(this.ctx, this.canvas.width + 230, this.canvas.height - 120))
    ...
    if(this.polices.length == 0) {
        this.polices.push(
            new Police(this.ctx, this.canvas.width + 800, this.canvas.height - 120)
            )
    }
    ...
    this.polices.forEach(police => {
        ...
        if(police.x < 0) {
            this.polices.shift()
            this.polices.push(new Police(this.ctx, this.canvas.width + 710, this.canvas.height - 120))
        }
    })
